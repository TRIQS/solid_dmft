...
from triqs.gf import MeshDLRImFreq, Gf, BlockGf, make_gf_dlr

from solid_dmft.gw_embedding.bdft_converter import convert_screened_int, calc_Sigma_DC_gw
...
class test_gw_embedding(unittest.TestCase):
    ...
    def test_calc_Sigma_DC_gw(self):
        ...
        Sig_DC_dlr, Sig_DC_hartree = calc_Sigma_DC_gw(Wloc_dlr, Gloc_dlr, Vloc)
        
        DC_ref = np.array([[ 7.52476548-0.0j , 0.0+0.0j, 0.0+0.0j],
                           [0.0+0.0j,  7.52236424+0.0j      , 0.0+0.0j],
                           [0.0+0.0j, 0.0+0.0j, 7.52279619+0.0j]])
