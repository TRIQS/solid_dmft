...
# try ForkTPS import
ftps = importlib.util.find_spec("forktps") is not None
if not ftps:
    mpi.report('ImportWarning: ForkTPS needs to be installed to run this test')
    sys.exit()
...
if mpi.is_master_node():
    with HDFArchive('out/inp.h5', 'r')['DMFT_results']['last_iter'] as out, \
        ...
        for key in ['G0_freq_0', 'Gimp_freq_0', 'Gimp_time_0', 'Sigma_freq_0']:
            print(key)
