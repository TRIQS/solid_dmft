...
It also changes the results of calculations that use `dc_orb_shift` on
orbitals that are degenerate in DFT, since the orbital dependent part of the
shift was previously averaged out of the impurity problem, see the fix below.
...
### test
* raise `n_cycles_tot` in `svo_cthyb_basic_tf` from 1e+4 to 4e+4. At the lower setting the self-energy was noisy enough that the run intermittently died inside the chemical potential search, on the tail fit of the lattice Green function in triqs' `density()` ("High frequency moments have an error greater than 1e-2") or on cthyb's `is_gf_hermitian` assertion. Note that pinning `random_seed` does not make such a test reproducible, see TRIQS/cthyb#193

### fix
* `dc_orb_shift` no longer gets averaged out by the orbital symmetrization. The degenerate solver blocks are found from the DFT density matrix before the shift is applied, so orbitals that are shifted differently were still symmetrized: G0 lost the orbital dependent part of the shift while the impurity levels kept it. With `delta_interface = true` this put spikes of the wrong sign (of order 10 for shifts of 0.1 eV) into Delta(tau) at tau=0 and beta and gave cthyb an average sign of about 1e-2; with `delta_interface = false` the impurity problem silently did not see the orbital dependent part of the shift. Blocks now stay degenerate only if their orbitals are shifted by the same amounts, so up and down remain degenerate in a paramagnetic calculation
...
* `plot_correlated_bands` warns instead of aborting where the result may not be what is expected: band disentanglement (`n_bands > n_orb`), where A(k,w) is the spectral function of the Wannier model rather than of the DMFT lattice problem and the chemical potential was converged for the electron count of the full outer window; an archive from a non-wannier90 converter, where the correlated orbitals need not come first; and a mismatch between the crystal-field eigenvalues of the archive and of the tight-binding H(R=0), which catches a grossly wrong orbital mapping also in Wannier basis. Being eigenvalue based, that last check cannot see a wrong order within a shell, nor a wrong set of orbitals when both the selected and the correct block are internally degenerate. `add_spin=True` together with a self-energy read from the archive is refused rather than guessed, since the correlated shell then appears in both spin blocks
