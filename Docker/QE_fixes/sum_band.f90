     ...
     SUBROUTINE sum_band_k()
       ...
       k_loop: DO ik = 1, nks
          ...
          IF ( dmft .AND. .NOT. dmft_updated) THEN
             ! 
             write(*,*) 'ZGEMMing with T mode' 
             DO j = 1, npw
                CALL ZGEMM('T', 'N', nbnd, 1, nbnd, (1.0d0,0.d0), v_dmft(:,:,ik), nbnd, evc(j,:), nbnd, (0.d0,0.d0), evc(j,:), nbnd)
             ENDDO
             !
             IF ( nks > 1 ) &
...
!
          ...
          ! zero out band energy if computing DMFT correction
          !IF (dmft) THEN
          !      WRITE(*,*) 'DMFT correction detected, band energy is computed at the nscf level', eband 
          !ENDIF
          !
          IF( use_tg ) THEN
             !
